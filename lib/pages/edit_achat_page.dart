...
class _EditAchatPageState extends State<EditAchatPage> {
  ...
  Future<void> _submit() async {
    ...
    try {
      // Mise à jour de l'achat dans Firestore
      await FirebaseFirestore.instance.collection('achats').doc(widget.achatId).update({
  ...
  void initState() {
    super.initState();
    // Chargement des données de l'achat existant
    final data = widget.achatData;
    _selectedFournisseurId = data['fournisseurId'];
    _selectedDate = (data['date'] as Timestamp).toDate();
    _dateController.text = DateFormat('yyyy-MM-dd').format(_selectedDate!);
    
    // Convertir les articles en maps
  ...
  Future<void> _loadFournisseurNom() async {
    if (_selectedFournisseurId != null) {
      try {
        final snapshot = await FirebaseFirestore.instance
            .collection('fournisseurs')
            .doc(_selectedFournisseurId)
            .get();
        if (snapshot.exists) {
          setState(() {
            _selectedFournisseurNom = snapshot.data()!.containsKey('societe') 
                ? snapshot.data()!['societe'] 
                : snapshot.data()!['nom'];
          });
        }
      } catch (e) {
        print('Erreur lors du chargement du nom du fournisseur: $e');
      }
    }
  }
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              ...
              ElevatedButton.icon(
                onPressed: () {
                  setState(() {}); // Force le rebuild et recalcule le total
                },
              ...
              ElevatedButton.icon(
                onPressed: () async {
                  ...
                  // Sauvegarder le PDF sur le téléphone
                  try {
                ...
                icon: const Icon(Icons.print, color: Colors.white,),
                 label: const Text("Imprimer la facture", style: TextStyle
              ...
              _isLoading
                  ...
                  : ElevatedButton(
                      ...
                      child: const Text("Enregistrer les modifications", style: TextStyle(fontSize: 16, color: Colors.white)),
                    ),
            ],
          ),
        ),
